namespace WinFormsApp1
{
    public partial class Form1 : Form
    {
        ...
        public static async Task<string> SendGetRequestAsync(string endpoint)
        {
            ...
            {
                var response = await client.GetAsync(endpoint); // buradaki getAsync, metod ismidir postta postAsync, putta putAsync kullanýlýr. ve endpoint urlimizi gönderiyoruz.
                var responseContent = await response.Content.ReadAsStringAsync(); // gelen cevabýn içeriðini response content deðiþkenine atýyoruz.
